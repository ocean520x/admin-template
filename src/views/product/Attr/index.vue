...
export default {
  ...
  methods: {
    ...
    addAttrValue() {
      //向属性值的数组里面添加元素
      //attrId:是你相应属性的id，目前而言我们是添加属性的操作，还没有相应的属性id，目前而言带给服务器的id为undefined
      //valueName：相应的属性值的名称
      this.attrInfo.attrValueList.push({
        attrId: this.attrInfo.id,
        valueName: "",
        flag: true, //给每一个属性值添加一个标记flag，用户切换查看模式与编辑模式，好处，每一个属性值可以控制自己的模式切换
      });
      this.$nextTick(() => {
    ...
    toLook(row) {
      ...
      let isRepeat = this.attrInfo.attrValueList.some((item) => {
        //需要将row从数组里面判断的时候去除
        //row最新新增的属性值【数组的最后一个元素】
        //判断的时候，需要把已有的数组当中新增这个属性值去除
        if (row !== item) {
    ...
    toEdit(row, index) {
      row.flag = true;
      //获取input节点，实现自动聚焦
      //点击span的时候，切换为input变为编辑模式，但是需要注意，对于浏览器而言，页面重绘与重拍耗时间的
    ...
    async addOrUpdateAttr() {
      //整理参数:1.如果用户添加了很多属性值，且属性值为空不应该提交给服务器
      //提交给服务器中的数据不应该出现flag字段
